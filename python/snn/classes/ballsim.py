...
class BallSim_Motor:
    ...
    def step(self, spikes):
        ...
        for i in range(self.MOTOR_COUNT):
            # calculate forces
            f_spring = -self.K * (
                self.motor_pos[i, 2] - self.MOTOR_POS_REST[i, 2]
            )  # spring force
...
class BallSim_Sensor:
    ...
    def reset(self):
        self.counter = (
            0  # time step counter. For spike generation at different frequencies
        )
    ...
    * M 범위: 0.001 ~ 0.03 [kg]
    ...
    def __init__(self, STEPS_PER_SEC, M, MU, verbose=False):
        ...
        self.motor = BallSim_Motor(
            STEPS_PER_SEC=STEPS_PER_SEC, PLATE_SIDE=self.PLATE_SIDE
        )
