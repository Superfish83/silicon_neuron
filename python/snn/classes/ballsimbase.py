...
class BallSimBase:
    ...
    * ball_w[3]: angular velocity of the ball [rad/s]
    ...
    def reset_random(
        ...
        ball_pos += (
            self.R - np.dot(ball_pos, plate_n)
        ) * plate_n  # keep contact with the plate

        tmp = np.random.uniform(ball_v_range[0], ball_v_range[1], size=2)
        ball_v = np.array([tmp[0], tmp[1], 0])
        ball_v -= (
            np.dot(ball_v, plate_n) * plate_n
        )  # make velocity parallel to the plate
    ...
    def _update_ball(self):
        ...
        self.ball_v -= (
            np.dot(self.ball_v, self.plate_n) * self.plate_n
        )  # make velocity parallel to the plate
        self.ball_pos += self.ball_v * self.DT
        self.ball_pos += (
            self.R - np.dot(self.plate_n, self.ball_pos)
        ) * self.plate_n  # keep contact with the plate
    ...
    def _is_ball_on_plate(self):
        ...
        if (min(xs) <= self.ball_pos[0] <= max(xs)) and (
            min(ys) <= self.ball_pos[1] <= max(ys)
        ):
            return True
        else:
            return False
